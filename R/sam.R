...
read_sam <- function(directory="WBcod_2015_short", from_web=FALSE, user="user3") {
  ...
  rby <- dplyr::full_join(ssb,fbar, by="year")
  rby <- dplyr::full_join(rby,bio, by="year")
  rby <- dplyr::full_join(rby,rec, by="year")
  rby <- dplyr::full_join(rby,yield, by = "year")
  ...
  x <- dplyr::full_join(N,mort,by=c("year","age"))
  x$f[is.na(x$f)] <- 0
